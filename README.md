...
| 包含完整还原页 `index.html` | 开 | 生成像素级还原的整页 HTML+CSS，解压后双击即可在浏览器预览。关掉可显著减小包体积，但失去"先看见原样"的兜底 |
...
```
index.html           完整还原页 —— 解压后双击即可在浏览器预览
style.css            预览页样式
...
**想看采集下来的页面长什么样？** 先解压整个 ZIP，然后双击根目录的 `index.html`，即可在浏览器中看到还原效果。

必须解压后再打开 —— 直接双击 ZIP 里的文件，浏览器无法解析其中的相对路径，看到的是没有样式和图片的空白页。
...
| 5 | `index.html` | 仅在需要像素级对照时打开（给人看预览用，AI 一般不需要读） |
...
- **原页隐藏的元素会被显式隐藏**：它们仍保留在 HTML 里以维持结构完整，同时补上 `display: none` / `visibility: hidden` 规则。少了这一步，这些元素会以浏览器默认样式显示出来 —— 表现就是整页散落着原页看不到的下拉菜单、被撑成巨大黑块的无尺寸 SVG 图标
- 只记录**偏离 CSS 初始值**的属性，没写的属性就是浏览器默认值
- **包含伪元素样式**（`::before` / `::after`）：图标字体、下拉箭头、装饰元素大量挂在这里，是完整还原的关键
- **保留 CSS 自定义属性**（`--xxx`）：原页内联样式里的 `color: var(--accent)` 需要变量定义才能解析，因此只在"定义点"输出，避免继承导致每个元素都重复同一批变量
- 图标字体的 `content` 会转义成 `\XXXX` 形式，不依赖文件编码
...
- **图标字体若定义在跨域样式表里可能拿不到**：`document.styleSheets` 对跨域 CSS 不可读，此时既拿不到 `@font-face` 定义、也下载不到字体文件，图标会显示成空白或方框。判断方法：看产物里的 `fonts/` 目录是不是空的。
