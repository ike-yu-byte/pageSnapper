 ...
 *   full/index.html      完整还原页（兜底，AI 通常不需要读）
 *   full/style.css
 */
...
function cleanHtml(html, rewriter, imagePrefix) {
  ...
  return rewriteText(out, rewriter, imagePrefix);
}
...
| 5 | \`full/index.html\` | 完整还原页，**仅在需要像素级对照时打开** | ${stats.fullHtmlSize} |

> 前三步通常足以理解整个页面的设计系统。\`full/\` 体积大，不要让 AI 默认读取。
...
export function buildPackage(data, assetMap, fontMap) {
  ...
  for (const res of data.images || []) {
    ...
    if (!urlMap.has(res.url)) urlMap.set(res.url, 'images/' + res.fileName);
  }
  ...
  if (shellHtml) {
    const inner = cleanHtml(shellHtml, rewriter, imagePrefix);
    ...
    files.push({
      ...
      data: /<\/head>/i.test(inner)
        ...
        : '<!DOCTYPE html>\n<html><head><link rel="stylesheet" href="style.css"></head><body>\n' +
          blocks.map((b) => cleanHtml(b.html, rewriter, imagePrefix)).join('\n') + '\n</body></html>'
    });
  } else {
    files.push({
      path: 'full/index.html',
  ...
  if (faceList.length) {
    ...
    for (const face of faceList) {
      ...
      fullCss.push(rewriteFontFaces(css, fontMapByUrl, meta.url || ''));
    }
    fullCss.push('');
  }
  ...
  for (const b of blocks) {
    ...
    for (const r of buildRulesForElements(blockEls, pool)) {
      ...
      fullCss.push(rewriteText(r.body, rewriter, imagePrefix));
      fullCss.push('}');
      fullCss.push('');
    }
  }
  ...
  files.push({ path: 'full/style.css', data: fullCss.join('\n') });
 ...
 */
export function rewriteFontFaces(cssText, fontUrlMap, base) {
  if (!fontUrlMap || !fontUrlMap.size) return String(cssText);
  return String(cssText).replace(/url\((['"]?)([^'")]+)\1\)/g, (m, q, v) => {
    const raw = v.trim();
    ...
    return fileName ? 'url(' + q + '../fonts/' + fileName + q + ')' : m;
  });
}
