...
function cleanHtml(html, rewriter, imagePrefix) {
  ...
  // 外部资源引用一律移除：
  //   <link> 指向的 css / 字体 / 图标并没有被下载进包，留着只会 404；
  //   即便下载了也是压缩丑化过的代码，交给 AI 无法拆分还原。
  //   <style> 是原页样式源码，而已由 computed style 生成的 style.css 完整覆盖，
  //   留着还可能因选择器特异性产生冲突。
  // 唯一需要保留的 <link rel="stylesheet" href="style.css"> 是在本函数之后注入的。
...
function orderKeys(obj) {
  return Object.keys(obj).sort((a, b) => {
    // CSS 自定义属性（--xxx）排最前，便于阅读
    const aVar = a.charCodeAt(0) === 45 && a.charCodeAt(1) === 45;
    const bVar = b.charCodeAt(0) === 45 && b.charCodeAt(1) === 45;
...
/**
 * content 里的私用区字符（图标字体）需要转义成 \XXXX 形式。
 * getComputedStyle 返回的是解析后的真实字符，直接写进 CSS 就依赖文件编码；
 * 转义后无论编码如何都能正确还原，也不会被编辑器/工具链弄坏。
 */
...
function declText(style, pool, indent) {
  ...
  for (const k of orderKeys(style)) {
    ...
    let v = resolveValue(k, style[k], pool);
    if (v === undefined || v === null || v === '') continue;
    if (k === 'content') v = escapeCssContent(v);
...
function buildRulesForElements(elements, pool) {
  ...
  const addRule = (style, selector) => {
    if (!style) return;
    const sig = orderKeys(style).map((k) => k + '\u0000' + style[k]).join('\u0001');
    if (!groups.has(sig)) groups.set(sig, { style, selectors: [] });
    groups.get(sig).selectors.push(selector);
  };

  for (const e of elements) {
    addRule(e.s, e.p);
    // 伪元素样式必须一并输出：图标字体的 content / font-family、装饰元素、
    // 下拉箭头等大量视觉细节都在 ::before / ::after 上。
    // 早先漏了这两行，导致它们全部在还原页里消失。
    if (e.b) addRule(e.b, e.p + '::before');
    if (e.a) addRule(e.a, e.p + '::after');
  }
...
export function buildPackage(data, assetMap, fontMap) {
  ...
  // ===== 原页面中不可见的元素 =====
  // 它们被采集时过滤掉了（因此没有任何规则），但 HTML 是整体复制的、它们仍在里面。
  // 不加约束就会以浏览器默认样式显示出来：整屏散落的下拉菜单、被撑成巨大黑块的
  // 无尺寸 SVG 图标，都是这么来的。这里显式把它们隐藏回去。
  const hiddenEls = data.hidden || [];
  if (hiddenEls.length) {
    fullCss.push('/* ===== 原页面中不可见的元素 =====');
    fullCss.push(' * 保留在 HTML 中以维持结构完整，但必须显式隐藏，');
    fullCss.push(' * 否则会以浏览器默认样式显示出来。 */');
    fullCss.push('');

    // 按声明合并，避免为几百个元素各写一条规则
    const byDecl = new Map();
    for (const h of hiddenEls) {
      const decls = [];
      if (h.d === 'none') decls.push('display: none');
      if (h.v === 'hidden') decls.push('visibility: hidden');
      if (h.o) decls.push('opacity: 0');
      if (!decls.length) continue;
      const key = decls.join('; ');
      if (!byDecl.has(key)) byDecl.set(key, []);
      byDecl.get(key).push(h.p);
    }

    for (const [decl, selectors] of byDecl) {
      fullCss.push(selectors.map((s) => rewriteText(s, rewriter, rootPrefix)).join(',\n') + ' {');
      fullCss.push('  ' + decl + ';');
      fullCss.push('}');
      fullCss.push('');
    }
  ...
  // ===== 原页面的伪元素规则（滚动条 / placeholder 等）=====
  // 这些伪元素取不到 computed style，是把原页 CSSOM 的规则原文搬过来的。
  // 导出的 HTML 保留了原页 class，所以这些选择器在还原页里依然能匹配。
  const pseudoRules = data.pseudoRules || [];
  if (pseudoRules.length) {
    fullCss.push('/* ===== 原页面的伪元素规则 =====');
    fullCss.push(' * 滚动条、placeholder、selection 这类样式无法从 computed style 取得，');
    fullCss.push(' * 这里沿用原页 CSSOM 中的规则原文。 */');
    fullCss.push('');
    for (const rule of pseudoRules) {
      fullCss.push(rewriteText(rule, rewriter, rootPrefix));
      fullCss.push('');
    }
  }
